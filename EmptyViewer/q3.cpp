...
void create_scene() {
    ...
    int top = gNumVertices - 2;
    for (int i = 0; i < width; ++i) {
        gIndexBuffer[t++] = top; gIndexBuffer[t++] = i; gIndexBuffer[t++] = (i + 1) % width;
    }
    int bottom = gNumVertices - 1;
    int base = (height - 3) * width;
    for (int i = 0; i < width; ++i) {
        gIndexBuffer[t++] = bottom; gIndexBuffer[t++] = base + (i + 1) % width; gIndexBuffer[t++] = base + i;
    }
}
...
mat4 get_view_matrix() { return mat4(1.0f); }
...
vec3 compute_color_pixel(const vec3& position, vec3 normal) {
    ...
    float diffuse = std::max(dot(normal, L), 0.0f);
...
void rasterize_triangle_phong(vec4 v0, vec4 v1, vec4 v2,
    ...
    for (int y = minY; y <= maxY; ++y) {
        for (int x = minX; x <= maxX; ++x) {
            ...
            if (z < depth_buffer[idx]) {
                depth_buffer[idx] = z;
                vec3 world_pos = alpha * w0 + beta * w1 + gamma * w2;
                vec3 normal = normalize(alpha * n0 + beta * n1 + gamma * n2);
                vec3 color = compute_color_pixel(world_pos, -normal);
...
void render_scene() {
    ...
    for (int i = 0; i < gNumTriangles; ++i) {
        ...
        vec3 n0 = normalize(N * vec3(m0));
        vec3 n1 = normalize(N * vec3(m1));
        vec3 n2 = normalize(N * vec3(m2));

        vec4 v0 = MVP * vec4(p0, 1.0f); v0 /= v0.w;
        vec4 v1 = MVP * vec4(p1, 1.0f); v1 /= v1.w;
        vec4 v2 = MVP * vec4(p2, 1.0f); v2 /= v2.w;

        rasterize_triangle_phong(v0, v1, v2, p0, p1, p2, n0, n1, n2);
    }
}
